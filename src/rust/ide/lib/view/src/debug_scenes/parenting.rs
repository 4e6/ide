...
//!
//! The scene should show a node with the text "empty" that has a red rectangle attached (so if
//! you move the node, the red rectangle also moves). Nothing else should be in the scene.
//!
//! During the setup phase we create two other rectangles, one blue and one green that get attached
//! and detached from the red rectangle. Due to the buggy behaviour we see a blue rectangle in the
//! scene. The rectangle is not attached to the node (does not move with the node).
//!
...
fn init(app:&Application) {
    ...
    node.set_position_xy(Vector2(-200.0,0.0));
